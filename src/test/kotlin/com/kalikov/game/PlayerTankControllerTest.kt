...
class PlayerTankControllerTest {
    ...
    fun beforeEach() {
        ...
        controller = PlayerTankController(game.eventManager, mock(), mock(), tank.player)
    }
...
//        controller.notify(GamePlayerTanksManager.PlayerTankCreated(tank))
...
//        assertSame(tank, controller.tank)
    ...
    fun `should recreate controller on subsequent player tank creation`() {
        ...
        controller.notify(GamePlayerTanksManager.PlayerTankCreated(tank1))

        val tank2 = stubPlayerTank(game, player = tank.player)

        controller.notify(GamePlayerTanksManager.PlayerTankCreated(tank2))
