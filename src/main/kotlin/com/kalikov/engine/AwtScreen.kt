...
import com.kalikov.game.horizontal
import com.kalikov.game.vertical
...
open class AwtScreen(private val frame: JFrame, private val fonts: AwtFonts) : Screen {
    companion object {
        val SCREEN_BG_COLOR: Color = Color.BLACK
    }

    override val surface: AwtScreenSurface get() = surfaceInstance

    private val surfaceInstance: AwtScreenSurface
    ...
    init {
        surfaceInstance = createSurface(Globals.CANVAS_WIDTH, Globals.CANVAS_HEIGHT)

        updateScale(surfaceInstance.width, surfaceInstance.height)
    }

    fun resize(): Boolean {
        return updateScale(surfaceInstance.width, surfaceInstance.height)
    }
    ...
    override fun clear() {
        surfaceInstance.clear(ARGB.rgb(SCREEN_BG_COLOR.rgb))
    }
            ...
            do {
                do {
                    ...
                    try {
                        ...
                        gfx.drawImage(surfaceInstance.image, 0, 0, null)
                    } finally {
                        gfx.dispose()
                    }
    ...
    override fun createSurface(): AwtScreenSurface {
        return AwtScreenSurface(fonts, surfaceInstance.width, surfaceInstance.height)
    }
